...
A requirement may list one position or 100+ positions. Preserve every distinct position — never silently drop one, and never merge two positions that are technically different trades (e.g. "Fitter" and "Pipe Fitter" are related but distinct; "Welder" and "6G Welder" are distinct skill levels).
NEVER aggregate a list into a single summary entry such as "Various Positions", "Multiple Roles", or "129 Nos" — that destroys the recruiter's data. If the source lists divisions, departments, or service lines each with a headcount (e.g. "Sperry Drilling Services – 16", "Wireline & Perforating – 18"), each line IS a position entry: extract every one individually with its own title (the division/service-line name, verbatim) and its own quantity (that line's number). A 19-line division list must produce 19 position entries, not one.
...
Overseas recruitment tables commonly show a GRADUATED PAY SCALE: the SAME position title and headcount, with several experience bands each paying a different salary (e.g. "8 yrs to < 9 yrs: SAR 10,000", "9 yrs to < 10 yrs: SAR 11,000", "10 yrs to < 11 yrs: SAR 12,000", "11 yrs & above: SAR 13,000"). This is ONE position entry, not one entry per band. Put every band into that position's salaryTiers array (one {experience, salary} pair per band, verbatim). NEVER create a separate position entry per experience/salary band, and NEVER repeat that position's full headcount for each band — the headcount belongs once, to the one position entry. Only use the plain salaryAmount/salaryCurrency fields when the source gives a single flat salary for the whole position; leave salaryTiers empty in that case. When salaryTiers is populated, leave that position's own experience field null — the experience bands already live inside salaryTiers, so repeating one of them in experience would be redundant, not additive.
