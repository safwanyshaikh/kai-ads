...
export function buildVisualQaInstructions(): string {
  return [
    ...
    "INFORMATION DEDUPLICATION — the Constitution requires a single integrated trust footer. Agency name, logo, RA number, MEA badge, and registration should appear ONCE in the footer/verification band, NOT repeated at the top of the advertisement. If you see the agency identity dominating the top AND appearing again in the footer, flag it as a defect: premium candidate-attention canvas is wasted on redundant trust elements. Exception: DTP/NEWSPAPER may show the agency masthead at top by justified print convention.",
