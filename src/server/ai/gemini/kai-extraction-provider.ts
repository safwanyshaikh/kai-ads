...
import { runKaiGeminiExtraction, type KaiExtractionOutcome } from "./kai-extraction-engine";
...
/**
 * Gemini equivalent of openai/kai-extraction-provider.ts — implements the
 * exact same seven Sprint 002 interfaces (plus the optional 8th,
 * CompositeExtractionProvider) against one shared Gemini call, with the
 * same per-input memoization so calling several of them for one input
 * triggers exactly one real API call.
 */
export class KaiGeminiExtractionProvider
  implements
    RequirementExtractionProvider,
    TradeSummaryProvider,
    IndustryDetectionProvider,
    ...
    CompositeExtractionProvider
{
  ...
  /** Exposed so callers can get token/latency usage for cost tracking without a second call. */
  async runComposite(input: AiExtractionInput): Promise<KaiExtractionOutcome> {
    const key = cacheKey(input);
    const cached = this.cache.get(key);
    if (cached) return cached;

    const promise = runKaiGeminiExtraction({
      text: input.text,
