 ...
 * Applied BEFORE any fact reaches GPT/the renderer, so the fact itself
 * already carries the correct currency — nothing downstream has to
 * invent or infer it. A figure that already states a currency (source-
 * given) is left completely untouched; this only fills in a currency
 * that was never stated, using the country the recruiter already gave.
 */
